...
import {
  ...
  hasOpenApiEnv, fetchSearchTrend, computeTrends,
} from './naver.js';
...
class NaverSearchTrendProviderImpl extends BaseProvider {
  constructor() {
    const ready = hasOpenApiEnv();
    super({
      ...
      connected: ready,
      note: ready
        ? '최근 3·12개월 검색 증감률을 자동 수집합니다 (상대지수 기반)'
        : '데이터랩이 등록된 기존 앱의 Client ID/Secret 을 등록하면 켜집니다',
    });
  }

  async fetchShoppingMetrics() { throw new NotConnectedError('네이버 데이터랩 쇼핑인사이트'); }

  /** 증감률만 가져온다 — 상대지수라 검색량 자체로는 쓸 수 없다 */
  async fetchKeywordMetrics(keyword) {
    if (!hasOpenApiEnv()) throw new NotConnectedError(this.name);
    if (!keyword) throw new Error('대표 키워드가 없습니다.');

    const points = await fetchSearchTrend(keyword);
    if (points.length === 0) {
      throw new Error(`"${keyword}" 의 검색 추세 데이터가 없습니다. (데이터랩 응답 0건)`);
    }
    return { points, months: points.length };
  }

  normalizeMetrics({ points } = {}) {
    return computeTrends(points);
  }
}

export const NaverSearchTrendProvider = new NaverSearchTrendProviderImpl();
