...
export async function fetchAllGuides(limit = 100) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/guides?select=slug,category,title,description,read_time,published_at,updated_at,content_type&order=created_at.desc&limit=${limit}`,
...
export async function fetchGuidesByCategory(category, limit = 100) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/guides?select=slug,category,title,description,read_time,published_at,updated_at,content_type&category=eq.${encodeURIComponent(category)}&order=published_at.desc&limit=${limit}`,
      { headers: getHeaders(), cache: "no-store" }
    );
    if (!res.ok) return [];
    const rows = await res.json();
